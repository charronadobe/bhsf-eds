...
export default async function decorate(block) {
  ...
  const shouldHide =
    hostname.includes("aem.live") || hostname.includes("aem.page");
  ...
  if (inputsArray.length < 2) {
    console.log(
      "Missing inputs, expecting 2, ensure both the image and DM URL are set in the dialog"
    );
    return;
  }
  ...
  if (deliveryType != "na") {
    if (deliveryType === "dm") {
      ...
      if (typeof s7responsiveImage !== "function") {
        console.error(
          "s7responsiveImage function is not defined, ensure script include is added to head tag"
        );
        return;
      }
      ...
      if (!imageEl) {
        console.error(
          "Image element not found, ensure it is defined in the dialog"
        );
        return;
      }
      ...
      if (!imageSrc) {
        console.error(
          "Image element source not found, ensure it is defined in the dialog"
        );
        return;
      }
    ...
    if (deliveryType === "dm-openapi") {
      ...
      if (!baseUrl) {
        console.error(
          "OpenAPI delivery URL not found. Ensure the DM delivery repository asset is selected."
        );
        return;
      }
      ...
      if (rotationVal && rotationVal.toLowerCase() !== "none")
        params.set("rotate", rotationVal);
